...
export const ImagesTabView = ({ initialQuery, onBackToAnswer }) => {
  ...
  return (
    <div className="p-4 bg-gray-900 text-white min-h-screen">
      ...
      {error && <p className="text-center text-red-500">Error: {error}</p>}
      {!loading && !error && images.length === 0 && (
        <p className="text-center text-gray-400">No images found for "{currentQuery}".</p>
      )}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
            {images.map((image, index) => (
              <button
                ...
                className="relative rounded-lg overflow-hidden shadow-lg group transform transition-all hover:scale-105 p-0 aspect-square focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-gray-900"
              >
                <img
                  src={image.thumbnailUrl || image.imageUrl}
                  alt={image.title || `Image from ${image.source}`}
